...
void bluetoothControl(byte inR1, byte inR2, byte inL1, byte inL2, char sig)
{
  ...
  {                                        // Forward Left - without PWM, it's Spin Left
    robotMover(inR1, inR2, inL1, inL2, 3); // stop
  }

  else if (sig == 'I')
  {                                        // Forward Right - without PWM, it's Spin Right
...
void robotMover(byte inR1, byte inR2, byte inL1, byte inL2, byte action)
{
  ...
  */
  ...
  case 4: // quay phải
    ...
    break;
  case 5: // rẽ trái
    motorControlWithSpeed(inR1, inR2, 1, 0, 120);
    motorControlWithSpeed(inL1, inL2, 0, 100, 0); // weak momentum
    break;
  case 6: // rẽ phải
